...
| `kit:startable-tickets` | What makes a ticket one an agent may pick up unbidden — the `ready-for-agent` candidate query, the five conditions, and what `epic`, `kit-blocked`, and the `kit-blocked-by` marker each mean. One home for a rule two callers read: `/kit:ship-ticket`'s sweep, which takes the lowest and reports the rest, and `/kit:list`, which prints the list. A second copy would drift, and the first symptom is a list promising a ticket the sweep then refuses. |
