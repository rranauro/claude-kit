...
| `kit:rails-load-bearing-specs` | The axis for judging one RSpec example: whether breaking the code it covers would break it. Three categories that convict a deletion candidate, each with the evidence it needs — a tautology is quotable from inside the example, a dead-code claim needs a **runtime witness** because `grep` cannot find a callback registered by symbol or an action reached only through the router, and a requirement claim convicts on contradiction rather than on nobody finding a reason. Its not-a-finding list is the load-bearing half, and the unit of a finding is a single `it` rather than a file. |
