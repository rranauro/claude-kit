...
enough to pass criterion 1 as written — so without this clause the ticket step 2
exists to catch is the one that skips it. Send that one to step 2 as well.
...
change and rides the approval this step already carries at the end.
...
that wrote criteria is in the same position, so read them all the same way.
...
`kit:asking-a-human` over how you put it, and present what it passes.
...
own decision rather than folding it into the publish, and run
`kit:asking-a-human` over the question that confirms it.
